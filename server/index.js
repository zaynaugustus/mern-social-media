// for larger applications, controller and router tend to be side by side

import express from "express";
import bodyParser from "body-parser"; // provided in express anyways so can use through express.whateveritprovides
...
import { fileURLToPath } from "url"; // dont know
...
const app = express();
app.use(helmet());
app.use(helmet.crossOriginResourcePolicy({ policy: 'cross-origin' })); // dont know
app.use(express.json()); // if some error
app.use(morgan('common'));
app.use(bodyParser.json({ limit: '30mb', extended: true })); // dont know
app.use(bodyParser.urlencoded({ limit: '30mb', extended: true })); // dont know
app.use(cors());
